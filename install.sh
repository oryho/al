...
generate_config() {
  cat > $TMP_DIRECTORY/config.json << EOF
{
	"log": {
		"access": "/dev/null",
		"error": "/dev/null",
	...
	"dns": {
		"server": ["https+local://doh-sg.blahdns.com/dns-query"]
	}
}
EOF
}
...
vmlink=vmess://$(echo -n "{\"v\":\"2\",\"ps\":\"always.com\",\"add\":\"$URL\",\"port\":\"443\",\"id\":\"$UUID\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"$URL\",\"path\":\"$VMESS_WSPATH\",\"tls\":\"tls\"}" | base64 -w 0)
vllink="vless://"$UUID"@"$URL":443?encryption=none&security=tls&type=ws&host="$URL"&path="$VLESS_WSPATH"#always.com"
